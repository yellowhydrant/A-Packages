...
namespace A.BehaviourTree
{
    public class DoubleClickSelection : MouseManipulator {
        ...
        private void OnMouseDown(MouseDownEvent evt) {
            var graphView = target as ABehaviourTreeView;
            if (graphView == null)
                return;
        ...
        void SelectChildren(MouseDownEvent evt) {
            ...
            ANodeView clickedElement = evt.target as ANodeView;
            if (clickedElement == null) {
                var ve = evt.target as VisualElement;
                clickedElement = ve.GetFirstAncestorOfType<ANodeView>();
                if (clickedElement == null)
                    return;
            }

            // Add children to selection so the root element can be moved
            ABehaviourTree.Traverse(clickedElement.node, node => {
