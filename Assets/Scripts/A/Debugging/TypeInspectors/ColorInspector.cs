...
{
    public class ColorInspector : DebugFieldInspectorOfType<Color>
    {
        TextField[] textFields;
        int vectorSize { get; } = 4;

        const string letters = "RGBA";

        public ColorInspector(object target, FieldInfo info) : base(target, info)
        {
        }
        ...
        public override VisualElement CreateGUI()
        {
            ...
            for (int i = 0; i < vectorSize; i++)
            {
                ...
                textField.RegisterValueChangedCallback((ctx) =>
                {
                    ...
                    field.SetValue(target, value); // change to only change the rgba variables of the color
                });
            }
