...
{
    public class LinkView : GraphView
    {
        public new class UxmlFactory : UxmlFactory<LinkView, GraphView.UxmlTraits> { }
        ...
        public void PopulateView()
        {
            ...
            var nodes = this.nodes.ToList().Cast<LinkNode>().ToList();
            for (int x = 0; x < nodes.Count; x++)
            {
                for (int y = 0; y < nodes[x].link.outputGUIDs.Length; y++)
                {
                    ...
                    {
                        var inputs = inputNodes[z].inputContainer.Query<Port>().Where((p) => (p.node as LinkNode).link.inputGUIDs.Contains(outputGuid)).ToList();
        ...
        Port GeneratePort(LinkNode node, Direction dir, Port.Capacity capacity = Port.Capacity.Single)
        {
            return node.InstantiatePort(Orientation.Horizontal, dir, capacity, typeof(Vector2));
        }

        void RefreshNode(LinkNode node)
        {
        ...
        public Port AddPort(LinkNode node, string portValue, Direction direction)
        {
        ...
        GraphViewChange OnGraphViewChanged(GraphViewChange graphViewChange)
        {
            if (graphViewChange.elementsToRemove != null)
            {
                graphViewChange.elementsToRemove.ForEach(elem =>
                {
                    ...
                    if (edge != null)
                    {
                        var childNode = edge.input.node as LinkNode;
                        ...
                        {
                            var parentNode = edge.output.node as LinkNode;
            ...
            if (graphViewChange.edgesToCreate != null)
            {
                ...
                {
                    var node = edge.input.node as LinkNode;
            ...
            if (graphViewChange.movedElements != null)
            {
                ...
                {
                    var node = elem as LinkNode;
                    if (node != null)
                    {
                        node.link.nodePosition = node.GetPosition().position;
                    }
                    var group = elem as LinkNodeGroup;
                    if (group != null)
                    {
